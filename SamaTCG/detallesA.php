...
if($id == '' || $token == ''){
    ...
    if($token == $token_tmp){
        ...
        $sql = $con->prepare("SELECT count(id_accesorios) FROM accesorios WHERE id_accesorios=? AND activo=1");
        $sql->execute([$id]);
        
        if($sql->fetchColumn() > 0){
            $sql = $con->prepare("SELECT nombre, descripcion, precio FROM accesorios WHERE id_accesorios=? AND activo=1 LIMIT 1");
            $sql->execute([$id]);
            ...
            $dir_images = 'img/productos/accesorios/' . $id . '/';
        
            $rutaImg = $dir_images . 'accesorio.jpg';
        
            if (!file_exists($rutaImg)) {
                $rutaImg = 'img/productos/accesorios/no-photo.jpg';
            }
            ...
            if (is_dir($dir_images)) {
                ...
                foreach ($archivos as $archivo) {
                    if ($archivo != 'accesorio.jpg' && (strpos($archivo, 'jpg') || strpos($archivo, 'jpeg'))) {

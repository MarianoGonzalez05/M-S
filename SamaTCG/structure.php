...
$sql = $con->prepare("SELECT id_structure, nombre, precio FROM structure WHERE activo>=1");
$sql->execute();
...
<div class="yugioh-container" id="yugioh-card-container">
    <?php foreach ($resultado as $row) { $id = $row['id_structure']; $precio = number_format($row['precio'], 2, '.', ''); ?>
    <div class="yugioh-card">
        <a href="detalles.php?id=<?php echo $id; ?>&token=<?php echo hash_hmac('sha1', $id, KEY_TOKEN); ?>">
            <?php 
            $id = $row['id_structure'];
            $imagen = "img/productos/structure/" . $id . "/structure.jpg";
            if (!file_exists($imagen)) {
                $imagen = "img/productos/structure/no-photo.jpg";
            }
            ?>
